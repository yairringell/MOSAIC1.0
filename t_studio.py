...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QGraphicsScene, 
                           ...
                           QGraphicsPolygonItem, QFrame, QGraphicsEllipseItem)
from PyQt5.QtCore import Qt, QRectF, QPointF, QTimer, pyqtSignal
from PyQt5.QtGui import QBrush, QPen, QColor, QPixmap, QPainter, QTransform, QPainterPath, QCursor, QPolygonF, QFontMetrics, QFont
...
class CenterMarker(QGraphicsEllipseItem):
    """A red circle marker to indicate center point"""
    def __init__(self, x, y, radius=5):
        super().__init__(-radius, -radius, radius*2, radius*2)
        self.setPos(x, y)
        
        # Set appearance - red circle
        self.setPen(QPen(QColor(255, 0, 0), 2))  # Red border
        self.setBrush(QBrush(QColor(255, 0, 0, 128)))  # Semi-transparent red fill
...
class WorkspaceView(QGraphicsView):
    def __init__(self, main_window=None):
        ...
        self.center_mode = False  # Center mode for placing center markers
        self.center_marker = None  # Current center marker on the scene
        
        # Circle drawing from center variables
        self.is_drawing_circle_from_center = False  # Track if drawing circle from center
        ...
        # Create center cursor for center mode
        self.center_cursor = self.create_center_cursor()
    ...
    def create_center_cursor(self):
        """Create a red circle cursor for center mode"""
        # Create a 15x15 pixmap for the center cursor
        size = 15
        pixmap = QPixmap(size, size)
        pixmap.fill(Qt.transparent)  # Transparent background
    ...
    def showEvent(self, event):
        ...
        QTimer.singleShot(100, self.update_scale_bars)
        
        # Ensure scale bars are visible and on top
        if hasattr(self, 'horizontal_scale_bar') and hasattr(self, 'vertical_scale_bar'):
            self.horizontal_scale_bar.show()
            self.vertical_scale_bar.show()
            self.horizontal_scale_bar.raise_()
            self.vertical_scale_bar.raise_()
    
    def paintEvent(self, event):
        """Handle paint events and ensure scale bars stay on top"""
        super().paintEvent(event)
        
        # Ensure scale bars are always on top
        if hasattr(self, 'horizontal_scale_bar') and hasattr(self, 'vertical_scale_bar'):
            self.horizontal_scale_bar.raise_()
            self.vertical_scale_bar.raise_()
    ...
    def set_background_image(self, pixmap):
        ...
        # Explicitly position the background image at (0,0) to align with scale bars
        self.background_item.setPos(0, 0)
        
        self.scene.addItem(self.background_item)
        
        # Update scene rect to start at (0,0) and match image dimensions
    ...
    def set_solid_color_background(self, color):
        ...
        # Explicitly position the background at (0,0) to align with scale bars
        self.background_item.setPos(0, 0)
        
        self.scene.addItem(self.background_item)
        
        # Update scene rect to start at (0,0) and match background dimensions
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_T:
            ...
            # Cancel circle drawing if in progress
            if self.is_drawing_circle_from_center:
                if self.preview_circle:
                    self.scene.removeItem(self.preview_circle)
                    self.preview_circle = None
                self.is_drawing_circle_from_center = False
                self.circle_drawing_start_pos = None
        else:
    ...
    def set_center_mode(self, enabled):
        """Enable or disable center mode"""
        self.center_mode = enabled
        if enabled:
            self.setCursor(self.center_cursor)
        else:
            self.setCursor(Qt.ArrowCursor)
    
    def add_center_marker(self, x, y):
        """Add a center marker at the specified position"""
        ...
        self.center_marker = CenterMarker(x, y)
        self.scene.addItem(self.center_marker)
        return self.center_marker
    
    def remove_center_marker(self):
        """Remove the current center marker"""
        if self.center_marker:
            self.scene.removeItem(self.center_marker)
    ...
    def mousePressEvent(self, event):
        # Check if clicking on center marker for circle drawing
        if self.center_marker and event.button() == Qt.LeftButton:
            scene_pos = self.mapToScene(event.pos())
            items_at_pos = self.scene.items(scene_pos)
            
            # Check if clicking on the center marker
            for item in items_at_pos:
                if isinstance(item, CenterMarker):
                    # Start circle drawing from center
                    self.is_drawing_circle_from_center = True
                    self.circle_drawing_start_pos = item.pos()  # Use center marker position
                    return  # Don't process other events when starting circle drawing
        
        if self.center_mode and event.button() == Qt.LeftButton:
            # Center mode: place center marker and then exit center mode
            scene_pos = self.mapToScene(event.pos())
            self.add_center_marker(scene_pos.x(), scene_pos.y())
            
            # Automatically exit center mode after placing the marker
            self.center_mode = False
            self.setCursor(Qt.ArrowCursor)
            
            # Update the button state in the main window (keep button checked and text as "Remove Center")
            if self.main_window and hasattr(self.main_window, 'center_btn'):
                self.main_window.center_btn.setChecked(True)
                self.main_window.center_btn.setText("Remove Center")
            
            return  # Don't process other modes when in center mode
    ...
    def mouseMoveEvent(self, event):
        ...
        # Handle circle drawing from center marker
        if self.is_drawing_circle_from_center and self.circle_drawing_start_pos:
            current_pos = self.mapToScene(event.pos())
            
            # Calculate radius from center marker to current mouse position
            dx = current_pos.x() - self.circle_drawing_start_pos.x()
            dy = current_pos.y() - self.circle_drawing_start_pos.y()
            radius = math.sqrt(dx*dx + dy*dy)
            
            # Remove previous preview circle if it exists
            if self.preview_circle:
                self.scene.removeItem(self.preview_circle)
            
            # Create preview circle
            if radius > 0:
                self.preview_circle = QGraphicsEllipseItem(
                    self.circle_drawing_start_pos.x() - radius,
                    self.circle_drawing_start_pos.y() - radius,
                    radius * 2,
                    radius * 2
                )
                # Set preview circle appearance - thin gray line
                self.preview_circle.setPen(QPen(QColor(128, 128, 128), 1, Qt.DashLine))
                self.preview_circle.setBrush(QBrush(Qt.NoBrush))  # No fill
                self.preview_circle.setZValue(50)  # Below center marker but above other items
                self.scene.addItem(self.preview_circle)
    ...
    def mouseReleaseEvent(self, event):
        # Handle circle drawing from center marker
        if self.is_drawing_circle_from_center and event.button() == Qt.LeftButton:
            if self.circle_drawing_start_pos:
                current_pos = self.mapToScene(event.pos())
                
                # Calculate radius from center marker to release position
                dx = current_pos.x() - self.circle_drawing_start_pos.x()
                dy = current_pos.y() - self.circle_drawing_start_pos.y()
                radius = math.sqrt(dx*dx + dy*dy)
                ...
                # Create final circle if radius is reasonable
                if radius > 5:  # Minimum radius to avoid tiny circles
                    final_circle = QGraphicsEllipseItem(
                        self.circle_drawing_start_pos.x() - radius,
                        self.circle_drawing_start_pos.y() - radius,
                        radius * 2,
                        radius * 2
                    )
                    # Set final circle appearance - black line, no fill
                    final_circle.setPen(QPen(QColor(0, 0, 0), 2))  # Black pen, 2 pixel width
                    final_circle.setBrush(QBrush(Qt.NoBrush))  # No fill
                    final_circle.setZValue(10)  # Above background but below center marker
                    self.scene.addItem(final_circle)
...
class MainWindow(QMainWindow):
    ...
    def create_left_taskbar(self):
        ...
        # Add center marker button
        self.center_btn = QPushButton("Add Center")
    ...
    def toggle_center_mode(self):
        """Toggle center mode"""
        self.workspace.set_center_mode(self.center_btn.isChecked())
        if self.center_btn.isChecked():
            self.center_btn.setText("Remove Center")
        else:
